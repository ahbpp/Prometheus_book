...
Царапины могут выполняться недоверенными пользователями. По умолчанию целевой объект не должен предоставлять данные, которые олицетворяют другую цель. Опция `honor_labels` снимает эту защиту, как и некоторые настройки перемаркировки.  
